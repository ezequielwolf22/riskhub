...
def get_license(
    ...
    if not license:
        raise HTTPException(404, _t("licenses.not_found", lang))
    return license
...
def create_or_assign_license(
    ...
    if not org:
        raise HTTPException(404, _t("licenses.organization_not_found", lang))

    existing = lic_svc.get_license(db, org_id)
    if existing:
        raise HTTPException(400, _t("licenses.already_assigned", lang))
...
def check_license_status(
    ...
    if current_user.role != UserRole.SUPERADMIN and current_user.organization_id != org_id:
        raise HTTPException(403, _t("licenses.not_authorized", lang))
