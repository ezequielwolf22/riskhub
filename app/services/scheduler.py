...
def _run_alert_rules() -> None:
    """Evaluacion periodica de reglas de alerta activas (todas las categorias:
    riesgos, controles, incidentes, politicas/tareas, proveedores/TPRM, BCP y
    vigilancia normativa). La logica vive en alert_rules_service para que el
    scheduler y el endpoint manual /api/alerts/check-rules no diverjan."""
    from app.services.alert_rules_service import run_all_orgs
    run_all_orgs()
