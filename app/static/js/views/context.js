...
const ViewContext = {
  async render(main) {
    ...
    main.innerHTML = UI.sectionHeader(
      t('context.title'),
      t('context.subtitle')
    ) + '<div id="ctx-content">' + UI.notice(t('context.loading')) + '</div>';

    try {
      const ctx = await Api.context.get();
      const c = document.getElementById('ctx-content');

      const likLabels = [0,1,2,3,4].map(i => t(`context.likelihood_${i}`));
      const consLabels = [0,1,2,3,4].map(i => t(`context.consequence_${i}`));
      const impactDims = ['financial', 'operational', 'reputational', 'regulatory', 'safety'];
      const impactNames = {
        financial:     t('context.impact_financial'),
        operational:   t('context.impact_operational'),
        reputational:  t('context.impact_reputational'),
        regulatory:    t('context.impact_regulatory'),
        safety:        t('context.impact_safety'),
      };
      ...
      const methMap = {
        iso27005: t('context.methodology_iso'),
        magerit: t('context.methodology_magerit'),
        combined: t('context.methodology_combined'),
      };

      c.innerHTML = `
        <div class="card">
          <h3>${t('context.general_data')}</h3>
          <div class="modal-body">
            <div class="span2">
              <label>${t('context.org_name')}</label>
              <input id="f-org" value="${UI.esc(ctx.organization_name || '')}" ${ro}>
            </div>
            <div class="span2">
              <label>${t('context.isms_scope')}</label>
              <textarea id="f-scope" rows="2" ${roArea}>${UI.esc(ctx.scope || '')}</textarea>
            </div>
            <div class="span2">
              <label>${t('context.isms_boundaries')}</label>
              <textarea id="f-bounds" rows="2" ${roArea}>${UI.esc(ctx.boundaries || '')}</textarea>
            </div>
            <div>
              <label>${t('context.risk_appetite')}</label>
              <input type="number" min="0" max="8" id="f-appetite" value="${ctx.risk_appetite ?? 3}" ${ro}>
            </div>
            <div class="span2">
              <label>${t('context.methodology_label')}</label>
              ${!ro ? `
              <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:4px;">
                ${[
                  {v:'iso27005', label: t('context.methodology_iso'),      desc: t('context.methodology_iso_desc')},
                  {v:'magerit',  label: t('context.methodology_magerit'),  desc: t('context.methodology_magerit_desc')},
                  {v:'combined', label: t('context.methodology_combined'), desc: t('context.methodology_combined_desc')},
                ].map(m => `
              ...
              <div style="font-size:13px;padding:10px;background:var(--bg-2);border-radius:8px;">
                <strong>${methMap[ctx.methodology||'iso27005']}</strong>
              </div>`}
            </div>
          </div>
        </div>
        ...
        <div class="card" style="margin-top:16px;">
          <h3>${t('context.likelihood_section')}</h3>
          <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px;">
            ${t('context.likelihood_desc')}
          </p>
          <table class="data">
            <thead><tr><th style="width:140px;">${t('context.col_level')}</th><th>${t('context.col_description')}</th></tr></thead>
            <tbody>
              ${[0,1,2,3,4].map(i => `
                <tr>
        ...
        <div class="card" style="margin-top:16px;">
          <h3>${t('context.impact_section')}</h3>
          <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px;">
            ${t('context.impact_desc')}
          </p>
          ${impactDims.map(dim => {
        ...
        <div class="card" style="margin-top:16px;">
          <h3>${t('context.acceptance_section')}</h3>
          <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px;">
            ${t('context.acceptance_desc')}
          </p>
          <div>
            <label>${t('context.risk_appetite_max')}</label>
            <input type="number" min="0" max="8" id="f-acc-appetite" value="${acc.appetite_max_level ?? 3}" ${ro} style="max-width:100px;">
          </div>
          <div style="margin-top:12px;">
            <label>${t('context.acceptance_rules_label')}</label>
            <textarea id="f-acc-rules" rows="6" style="width:100%;font-size:13px;" ${roArea}>${UI.esc(rules)}</textarea>
          </div>
        </div>

        <div class="card" style="margin-top:16px;">
          <h3>${t('context.matrix_section')}</h3>
          <p style="color:var(--text-muted);font-size:13px;">
            ${t('context.matrix_desc')}
          </p>
          ${ViewContext._matrixHtml(ctx.risk_matrix)}
        </div>

        <div class="card" style="margin-top:16px;" id="rl-card">
          <h3>${t('context.risk_levels_section')}</h3>
          <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px;">
            ${t('context.risk_levels_desc')}
          </p>
          <div id="rl-body">${UI.notice(t('context.loading'))}</div>
        </div>

        ${isAdmin ? `
        <div style="margin-top:24px;text-align:right;">
          <button class="btn btn-primary" id="btn-save">${t('context.save_context')}</button>
        </div>` : ''}
      `;
      ...
      if (isAdmin) {
        document.getElementById('btn-save').onclick = async () => {
          try {
            ...
            UI.toast(t('context.updated'), 'success');
  ...
  _rlTableHtml(bands, isAdmin) {
    ...
    const _LABELS = [
      t('context.color_low'), t('context.color_medium'), t('context.color_high'),
      t('context.color_critical'), t('context.color_purple'),
    ];
      ...
      <tr data-rl-idx="${i}">
        ...
        <td>
          <input class="rl-label" value="${UI.esc(b.label)}" ${ro} style="width:90px;font-size:12px;"
                 placeholder="${t('context.rl_new_label')}">
        </td>
            ...
            ${(() => {
              ...
              return `
                <select class="rl-color" ${ro} style="font-size:12px;">
                  ...
                  <option value="__custom__" ${isCustom ? 'selected' : ''}>${t('context.rl_custom_color')}</option>
                </select>
        ...
        ${isAdmin ? `<td>
          <button class="btn btn-ghost btn-sm rl-del" title="${t('context.rl_del_title')}" data-rl-idx="${i}">&#10005;</button>
        </td>` : '<td></td>'}
      </tr>`).join('');
    return `
        ...
        <table class="data" style="min-width:560px;">
          <thead>
            <tr>
              ...
              <th>${t('context.col_code')}</th>
              <th>${t('context.col_label')}</th>
              <th>${t('context.col_range')}</th>
              <th>${t('context.col_color')}</th>
      ...
      <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap;">
        <button class="btn btn-ghost btn-sm" id="rl-add">${t('context.rl_add')}</button>
        <button class="btn btn-primary btn-sm" id="rl-save">${t('context.rl_save')}</button>
  ...
  _rlBindEvents(container, initialBands) {
    ...
    container.querySelector('#rl-add')?.addEventListener('click', () => {
      bands.push({
        code: t('context.rl_new_code'), label: t('context.rl_new_label'),
    ...
    container.querySelector('#rl-save')?.addEventListener('click', async () => {
      ...
      try {
        ...
        UI.toast(t('context.rl_saved'), 'success');
        bands = saved;
        refresh();
      } catch (e) {
    ...
    container.querySelector('#rl-reset')?.addEventListener('click', async () => {
      if (!await UI.confirm(t('context.rl_reset_confirm'))) return;
      try {
        await Api.risk_levels.reset();
        await RiskLevels.load();
        UI.toast(t('context.rl_reset_done'), 'success');
  ...
  _rlReadFromDom(container, bands) {
    ...
    rows.forEach((tr, i) => {
      ...
      if (!code || !label || isNaN(min) || isNaN(max) || !color) {
        UI.toast(t('context.rl_fill_all'), 'error');
        valid = false;
        return;
      }
      if (min > max) {
        UI.toast(t('context.rl_min_max_error', {label}), 'error');
        valid = false;
        return;
      }
  ...
  _matrixHtml(matrix) {
    if (!matrix) return `<p style="color:var(--text-subtle);">${t('context.matrix_none')}</p>`;
    const labelsX = [0,1,2,3,4].map(i => t(`context.matrix_like_${i}`));
    const labelsY = [4,3,2,1,0].map(i => t(`context.consequence_${i}`));
