"""Sprint 6 final integration test — regwatch propagation end-to-end.

Usa una BD SQLite aislada por test (no comparte estado con la sesion
principal de conftest). No requiere hashing de contrasenas reales.
"""
import os
import pytest
...
def sprint6_db(tmp_path_factory):
    """BD SQLite aislada para este modulo de test."""
    db_file = tmp_path_factory.mktemp("sprint6") / "sprint6.db"
...
def test_sprint6_regwatch_full_propagation(sprint6_db):
    """Verifica el pipeline completo: pack → propagacion → estado en BD."""
    db = sprint6_db

    org = Organization(name="Test Org Sprint6", plan="enterprise")
    db.add(org)
    db.flush()
    ...
    ctrl = Control(
        code="8.10-sprint6",
        name="Eliminacion de informacion",
    ...
    req = ComplianceFrameworkStatus(
        ...
        requirement_id="8.10-sprint6", status="implemented",
    )
    db.add_all([impl, pol, plan, req])
    db.commit()
    ...
    c  = q("SELECT description FROM controls WHERE code=:c", c="8.10-sprint6")
